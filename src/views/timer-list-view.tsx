...
import { TimerCard, TimerCardEmpty, TimerListItem } from './timer-card';
...
export function TimerListView({
  ...
  const newTimerHref = currentProjectId
    ? `/timers/new?projectId=${currentProjectId}`
    : '/timers/new';
  ...
  return (
    ...
    <div class="mb-6">
      ...
      <div class="mb-3 flex gap-2">
        ...
        <select
          x-on:change={`window.location.href = location.pathname + '?sort=' + $event.target.value + (location.search.replace(/[?&]sort=[^&]*/g, '').replace(/^\\?/, '&').replace(/^&/, '?'))`}
